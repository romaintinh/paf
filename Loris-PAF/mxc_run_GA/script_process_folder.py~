...
for root,dirs,files in os.walk(taskdirectory):
	...
	for root_path in root:
			...
			for files_name in files:
				...
				try :				
					begin = time.time()
					print ["doalarm","1800","python","mxc_run_ga.py",taskFilePath,resultExperimentFilePath]
					output = subprocess.check_output(["doalarm","1800","python","mxc_run_ga.py",taskFilePath,resultExperimentFilePath],stderr=subprocess.STDOUT)#,stderr=sys.stdout.fileno())
					end = time.time()
