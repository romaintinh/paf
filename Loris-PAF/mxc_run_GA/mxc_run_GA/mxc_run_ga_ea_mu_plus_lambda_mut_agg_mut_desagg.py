...
if len(sys.argv) > 2:
	...
	file_path_result = task_file_path+"_result_ea_mu_plus_lambda_agg_desagg.txt"
...
if nb_hi_tasks == 0 or nb_lo_tasks == 0:
	task_file_path_modal_server = task_file_path+"_modal_server_allocation_ea_mu_plus_lambda_agg_desagg.txt"
...
nb_generation = 100
global_variablesglobal_nb_aggregated_servers = 0
nb_modal_servers = nb_individus * nb_hi_tasks
...
toolbox.register("mutate_agg", mxc_run_ga_utils.mut_flip_bit_aggregation)
toolbox.register("mutate_desagg", mxc_run_ga_utils.mut_flip_bit_desaggregation)
...
def main():
	...
	global_variables.global_nb_aggregated_servers = 0
	# Evaluate the entire population
	fitnesses = list(map(toolbox.evaluate, pop))
	for ind, fit in zip(pop, fitnesses):
		ind.fitness.values = fit
	ratio_aggregated_servers = float(global_variables.global_nb_aggregated_servers) / float(nb_modal_servers)
	limit_agg_desagg = mxc_run_ga_utils.Compute_limit_for_agg_desagg(ratio_aggregated_servers)		
	print("ratio %s\tnb_aggregated %s\tlimit:%s" % (ratio_aggregated_servers,global_variables.global_nb_aggregated_servers,limit_agg_desagg))
	...
	for g in range(NGEN):
		...
		for x_offspring in xrange(nb_offspring):
			...
			if op_choice < CXPB:			# Apply crossover
				...
				agg_or_desagg = random.random()
				limit_agg_or_des_agg = mxc_run_ga_utils.Compute_limit_for_agg_desagg(ratio_aggregated_servers)
				ind = toolbox.clone(random.choice(pop))
				print "ind before mut"
				print ind
				if agg_or_desagg < limit_agg_or_des_agg:
					print "agg"
					toolbox.mutate_agg(ind)
					print ind
					print
				else:
					print "desagg"
					toolbox.mutate_desagg(ind)					
					print ind
					print
	...
	if mxc_run_ga_utils.Check_best_ind_fitness(best_ind.fitness.values) == True:
		...
		best_individual_generation_file_statistics = file_task_path_directory_hierarchy[:-1]+"_best_individual_statistics_ea_mu_plus_lambda_agg_desagg.csv"
